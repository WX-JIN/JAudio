...
public class WaveformView extends View {
    ...
    protected void computeDoublesForAllZoomLevels() {
        int numFrames = mSoundFile.getFrameCount();
    ...
    protected float getZoomedInHeight(float zoomLevel, int i) {
        int f = (int) zoomLevel;
        if (i == 0) {
            return 0.5f * getHeight(0, mSoundFile.getFrameCount(), mSoundFile.getFrameGains(), scaleFactor, minGain, range);
        }
        if (i == 1) {
            return getHeight(0, mSoundFile.getFrameCount(), mSoundFile.getFrameGains(), scaleFactor, minGain, range);
        }
        if (i % f == 0) {
            float x1 = getHeight(i / f - 1, mSoundFile.getFrameCount(), mSoundFile.getFrameGains(), scaleFactor, minGain, range);
            float x2 = getHeight(i / f, mSoundFile.getFrameCount(), mSoundFile.getFrameGains(), scaleFactor, minGain, range);
            return 0.5f * (x1 + x2);
        } else if ((i - 1) % f == 0) {
            return getHeight((i - 1) / f, mSoundFile.getFrameCount(), mSoundFile.getFrameGains(), scaleFactor, minGain, range);
        }
        return 0;
    }
    ...
    protected float getZoomedOutHeight(float zoomLevel, int i) {
        ...
        float x1 = getHeight(f, mSoundFile.getFrameCount(), mSoundFile.getFrameGains(), scaleFactor, minGain, range);
        float x2 = getHeight(f + 1, mSoundFile.getFrameCount(), mSoundFile.getFrameGains(), scaleFactor, minGain, range);
        return 0.5f * (x1 + x2);
    }

    protected float getNormalHeight(int i) {
        return getHeight(i, mSoundFile.getFrameCount(), mSoundFile.getFrameGains(), scaleFactor, minGain, range);
    }
